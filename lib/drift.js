...
exports.createDriftEvent = createDriftEvent;
const storage_1 = require("./storage");
/** Compare current snapshot against previous to detect drift */
function checkDrift(current, previous) {
    const changedFiles = [];
    const staleContextFiles = [];
    ...
    const criticalChanges = changedFiles.filter(f => criticalPatterns.some(p => f.path.includes(p)));
...
function createDriftEvent(projectId, result) {
    return {
