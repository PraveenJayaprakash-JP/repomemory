...
exports.auditClaudeMd = auditClaudeMd;
...
const hasAny = (text, keywords) => keywords.some((k) => lower(text).includes(k));
const countMatches = (text, keywords) => keywords.filter((k) => lower(text).includes(k)).length;
function clampScore(raw, max) {
    return Math.min(Math.max(Math.round(raw), 0), max);
}
// ── Dimension scorers ──────────────────────────────────────────────────
function scoreArchitecture(content, snapshot) {
    const max = 15;
    ...
    else if (folderHits >= 1) {
        points += 1;
        reasons.push('only ' + folderHits + ' folder path(s) mentioned');
    }
    // Module names from topFiles (4 pts)
    const topFileRefs = snapshot.topFiles.filter((f) => text.includes(lower(f)));
    if (topFileRefs.length >= 3) {
        points += 4;
    }
    else if (topFileRefs.length >= 1) {
        points += 2;
        reasons.push('only ' + topFileRefs.length + '/' + snapshot.topFiles.length + ' top files referenced');
    }
    else {
        reasons.push('no top files referenced from project');
    }
    ...
    if (hasAny(content, ['dependencies', 'packages', 'imports', 'requires'])) {
        points += 2;
    }
...
function scoreOffLimits(content) {
    const max = 15;
    const text = lower(content);
    let points = 0;
    const reasons = [];
...
function scoreDeployment(content) {
    const max = 10;
    const text = lower(content);
    let points = 0;
    const reasons = [];
...
function scoreFreshness(content, snapshot) {
    const max = 10;
    const text = lower(content);
    let points = 0;
    const reasons = [];
    ...
    if (keyFile.packageJson) {
        const pkg = keyFile.packageJson;
        if (pkg.name && text.includes(lower(String(pkg.name))))
            keyRefs++;
        if (pkg.scripts && typeof pkg.scripts === 'object')
            keyRefs++;
    }
    if (keyFile.dockerFile) {
        if (hasAny(content, ['docker', 'dockerfile']))
            keyRefs++;
    }
    if (keyFile.ciConfig) {
        if (hasAny(content, ['ci', 'pipeline', 'github actions']))
            keyRefs++;
    }
    if (keyFile.envExample) {
        if (hasAny(content, ['env', 'environment']))
            keyRefs++;
    }
    if (keyRefs >= 3) {
        points += 4;
...
// ── Main export ────────────────────────────────────────────────────────
function auditClaudeMd(content, snapshot) {
    ...
    const dimensions = DIMENSIONS.map((def, i) => {
